"""v18 single-card cfg, used for the ~5-min sanity smoke before 4-GPU DDP ship.

Inherits eloftr_full_v18_pose_msyn_rot50_only_ddp.py (which itself inherits
v14 ddp + 7 v18 overrides) and only adjusts LR / WARMUP so single-card bs=4
reaches the SAME TRUE_LR (1.25e-4) and actual-WARMUP-step count as the
4-GPU run. Math identical to v14 singlecard:
...
Note: H aug (cfg.DATASET.MGDPT_HOMOGRAPHY_AUG=True) is inherited from the
ddp parent; data.py train-only gate keeps val/test unaugmented even in the
single-card smoke.
"""
